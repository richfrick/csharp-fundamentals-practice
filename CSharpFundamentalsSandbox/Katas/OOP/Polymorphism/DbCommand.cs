...
namespace CSharpFundamentalsSandbox.Katas.OOP.Polymorphism
{
    ...
    //
    //Note that here, inside the DbCommand, we have a reference to DbConnection. Depending on the type of DbConnection sent at runtime,
    //opening and closing a connection will be different. For example, if we initialize this DbCommand with a SqlConnection,
    //we will open and close a connection to a Sql Server database. This is polymorphism. Interestingly, DbCommand doesn’t care about how a connection is opened or closed.
    //It’s not the responsibility of the DbCommand. All it cares about is to send an instruction to a database.
    //
    //For running the instruction, simply output it to the Console.In the real-world,
    //SQL Server (or any other DBMS) provides an API for running an instruction against the database. We don’t need to worry about it for this exercise.
    //
    ...
    public class DbCommand
    {
        ...
        {
            if (dbConnection == null) throw new InvalidOperationException("DbConnection not established");

            if (string.IsNullOrWhiteSpace(dbCommand)) throw new InvalidOperationException("valid DB c0mmand not set");
